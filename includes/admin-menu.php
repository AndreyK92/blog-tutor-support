...
function bt_custom_toolbar_links( $wp_admin_bar ) {
  ...
  if ( current_user_can('editor') || current_user_can('administrator') ) {
    ...
    // "Clear caches link to open the Support Hero widget
    function bt_clear_all_caches() {
      if ( function_exists( 'rocket_clean_domain' ) ) {
        rocket_clean_domain();
      }
    }
    if ( isset( $_GET['bt_clear'] ) ) {
      bt_clear_all_caches();
    }
    $args = array(
        'id' => 'bt-clear-caches',
        'title' => 'Clear WP Rocket and Cloudproxy caches',
        'href' => 'index.php?bt_clear=true',
        'parent'  => 'blog-tutor-menu',
        'meta' => array(
            'class' => 'btButton',
            'title' => 'Click to clear both WP Rocket and Cloudproxy(Sucuri) caches.',
            )
    );
